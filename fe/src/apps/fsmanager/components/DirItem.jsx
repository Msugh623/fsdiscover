...
const DirItem = ({ item }) => {
    const { key,setModal, title } = useFsContext()
    ...
    const dropDownContent = <div className=" p-1 small themebg rounded inner" onClick={() => {
        setHasDd(false)
    }} style={{
            position: 'relative',
            
        }}>
        {type == 'file' ? <div className="p-1 rounded active" title="Download File" style={{ cursor: 'pointer' }} onClick={() => {
        const a = document.createElement('a')
        a.href = location.pathname + '/' + psr?.name
        a.download = psr.name
        a.click()
    }}><FaDownload className='icon' /> Download File</div> :
        <div className="p-1 rounded active" title='Download as compressed ZIP' style={{ cursor: 'pointer' }} onClick={() => {
            const a = document.createElement('a')
            a.href = location.href.replace('/fsexplorer', '/zipper')
            a.download = psr.name
            a.click()
        }}><FaFileZipper className='icon' /> Download ZIP</div>
        }</div>
